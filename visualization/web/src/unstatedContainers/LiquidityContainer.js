...
class LiquidityContainer extends Container {
    ...
    async updateSelection(exchangeName, currencyPair, currencyCodePair, interval) {
        await this.setState({
            interval: interval,
        ...
        if (interval !== "now" && exchangeName && currencyPair) {
            const result = await CryptolpAxios.getLiquidityIndicators(exchangeName, currencyPair, interval);
            await this.setState({
