...
class Userimagetext extends Component {
    ...
    componentWillUnmount() {
        const index = CryptolpAxios.userUpdatedListners.indexOf(this.updateUserInfo);
    ...
    render() {
        return !this.props.isSlidePanel ? (
            <Dropdown
                ...
                image={<img src="/images/man2x.png" height="36" width="36" alt="" className="profile-image" />}>
                <div onClick={CryptolpAxios.logout} className="dropdown-text-inner currency">
                    Log Out
                </div>
            </Dropdown>
        ) : (
            <NavLink
                ...
                className={`nav-link w-nav-link ${this.props.open ? "w--nav-link-open" : ""}`}>
