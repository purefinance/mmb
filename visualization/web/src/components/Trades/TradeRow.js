...
function TradeRow(props) {
  const arrow = (
    <i
      className={`fas fa-angle-down icon-arrow-trades ${
        props.isSelect ? "select" : ""
      } cursor`}
      ...
      <Row style={props.style} className="base-row">
        {props.dashboard && (
          <React.Fragment>
            ...
            <Col className="base-col currency_pair_item">
              {props.transaction.currencyCodePair}
            </Col>
          </React.Fragment>
        )}
        <Col
        ...
        >
          {props.transaction.hedged === null
            ? "--"
            : (props.transaction.hedged * 100).toFixed(1) + "%"}
        </Col>
        <Col className="base-col ls_item">
          {props.transaction.profitLossPct === null
            ? "--"
            : props.transaction.profitLossPct.toFixed(3) + "%"}
        </Col>
        {props.dashboard && (
          <Col
            xs={props.dashboard ? 2 : 1}
            className="base-col currency_pair_item"
          >
        ...
        <Row
          className={`base-row trade_info ${props.isSelect ? "select" : ""}`}
        >
