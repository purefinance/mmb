...
function Indicators(props) {
    ...
    const hasBalanceIndicators = !loading && curIndicators && curIndicators.balanceIndicators;
    return (
        <Row className="base-container">
            <IndicatorErrorBoundary>
                ...
                {hasBalanceIndicators &&
                    curIndicators.balanceIndicators.map((indicator, index) => {
                        ...
                        if (limitAmount !== undefined) {
                            ...
                            percent = Math.max(0, Math.min(1, (filledAmount + reservationAmount) / limitAmount)) * 100;
                            tooltip += " / Limit";
                        }
                ...
                {hasBalanceIndicators && getLiquidationIndicator(curIndicators, props.orderState, props.symbol)}
            </IndicatorErrorBoundary>
        </Row>
    );
}
...
function getIndicator(curIndicators, index) {
    ...
    const filledAmount = indicator.currencyCode === amountCurrencyCode ? position : 0;
...
function getPositionCurrencyIndicator(curIndicators) {
    ...
    if (!ind1.limitAmount || !ind2.limitAmount) {
        ...
        const position = Math.max(-1, Math.min(1, ind2.normAmount - ind1.normAmount)) * 100;
        indicatorsData = `Relative: ${utils.round(position, 0)}%`;
    }
    return <Indicator key={3} data={indicatorsData} title={`${ind1.currencyCode}/${ind2.currencyCode} position`} />;
}
...
function getLiquidationIndicator(curIndicators, orderState, symbol) {
    ...
    const actualPrice = utils.round(orderState.indicators.mediumPrice, symbol.pricePrecision);
    ...
    if (avgEntryPrice < liquidationPrice) {
        percent = Math.max(0, (actualPrice - avgEntryPrice) / (liquidationPrice - avgEntryPrice)) * 100;
    } else {
        percent = Math.max(0, (avgEntryPrice - actualPrice) / (avgEntryPrice - liquidationPrice)) * 100;
    }

    const content = `${avgEntryPrice} / ${actualPrice} / ${liquidationPrice}`;
    const title = `Liquidation (${
        curIndicators.position.side === 1 ? "Buy" : curIndicators.position.side === 2 ? "Sell" : "None"
    })`;
    return (
        <Indicator
            key={4}
