...
async function checkParameters(props) {
    ...
    if (!exchangeState.exchanges || !exchangeState.exchangeName || !exchangeState.currencyPair) return;
    ...
    if (props.isNeedInterval) {
        ...
        newPath += interval && validIntervals.indexOf(interval) >= 0 ? `/${interval}` : "/day";
    }

    if (props.isNeedExchange && props.exchange.state.exchanges && exchangeState.exchangeName) {
        if (exchangeName && exchangeState.exchangeName !== exchangeName) {
            ...
            if (props.path === "/profits") {
                newPath +=
                    isValidExchange || exchangeName === "all" ? `/${exchangeName}` : `/${exchangeState.exchangeName}`;
            } else {
                const newExchange =
    ...
    if (props.isNeedCurrencyCodePair && props.exchange.state.symbols && exchangeState.currencyCodePair)
        if (currencyCodePair && exchangeState.currencyCodePair !== currencyCodePair) {
    ...
    if (newPath && props.history.location.pathname !== newPath) {
        ...
        if (exchangeName === "all" && props.path === "/profits" && props.exchange.state.exchangeName !== "all") {
            ...
            if (props.exchange.state.exchanges)
                props.exchange.state.exchanges.forEach(async (ex, index) => {
                    if (ex.name === exchangeName && props.exchange.state.exchangeName !== exchangeName) {
...
function toLocalDateTime(dateTime) {
    return parseDateTime(dateTime).toLocaleString(DateTime.DATETIME_SHORT_WITH_SECONDS);
}
