...
class VolumeNow extends React.Component {
    ...
    async updateSubscription() {
        ...
        if (exchangeState.exchangeName && exchangeState.currencyCodePair) {
            await ws.updateVolumeSubscription(exchangeState.exchangeName, exchangeState.currencyCodePair);
        }
    }
        ...
        return (
            <Container className="base-container">
                {this.props.ws.state.isConnected ? (
                    <Body volume={this.props.volume} exchange={this.props.exchange} ws={this.props.ws} />
                ) : (
                    <Spinner />
                )}
            </Container>
