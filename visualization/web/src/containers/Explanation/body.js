    ...
    if (explanations) {
      explanations.forEach((element, index) => {
        elements.push(
          <Row
            key={index}
            className={`base-row explanation ${
              this.state.selectIndex === index ? "selected" : ""
            }`}
            onClick={() => this.onSelect(index)}
          >
            {element.id}:{" "}
            {DateTime.fromISO(element.dateTime).toFormat("HH:mm:ss.SSS")}
          </Row>
        );
        forElements[index] = [];
        element.priceLevels.forEach((el, i) => {
          forElements[index].push(
            <Row
              key={i}
              className={`base-row explanation-row ${
                this.state.selectedIndexes[i] ? "high" : ""
              }`}
            >
              <Col md={1} className="base-col center arrowCol">
                <i
                  className={`fas fa-angle-down icon-arrow cursor`}
                  onClick={() => this.onSelectExplanation(i)}
                ></i>
              </Col>
