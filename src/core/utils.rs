...
impl AppendTable8 {
    ...
    pub fn add_or_get(&self, value: &str) -> u8 {
        ...
        let count = map.len();
        let index: u8 = count.try_into().with_expect(|| {
            format!(
                "For AppendTable8 index {} is greater then 255 (map: {:?})",
                count, map
            )
        });
    ...
    pub fn get_str(&self, index: u8) -> &'static str {
        ...
        inner // can read without lock because buffer is append only table
            .buffer
            .get(index as usize)
            .with_expect(|| format!("Index {} out of bounds of AppendTable8", index)) // Index should be correct because we get it when created table item instance
            .with_expect(|| {
                format!(
                    "Unable to get string from AppendTable8[{}] because it's None (table: {:?})",
                    index,
                    self.0.lock()
                )
            })
...
impl AppendTable16 {
    ...
    /// Return table index of string equal to specified value if found otherwise create leaked
    /// clone of string, add it to table and return index in table for new value
    pub fn add_or_get(&self, value: &str) -> u16 {
        let mut inner = self.0.lock();
        let map = &mut inner.map;
        ...
        let count = map.len();
        let index: u16 = count.try_into().with_expect(|| {
            format!(
                "For AppendTable16 index {} is greater then 65535 (map: {:?})",
                count, map
            )
        });
    ...
    pub fn get_str(&self, index: u16) -> &'static str {
        // SAFE: use unsafe just for access to item of append only table
        let inner = unsafe { self.0.data_ptr().as_ref() }
            .expect("Can't get pointer to AppendTable16Inner data");

        // Should exist because it assigned when created table item instance
        inner // can read without lock because buffer is append only table
            .buffer
            .get(index as usize)
            .with_expect(|| format!("Index {} out of bounds of AppendTable16", index)) // Index should be correct because we get it when created table item instance
            .with_expect(|| {
                format!(
                    "Unable to get string from AppendTable16[{}] because it's None (table: {:?})",
                    index,
                    self.0.lock()
                )
            })
