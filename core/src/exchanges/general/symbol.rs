...
impl Symbol {
    ...
    fn round_by_mantissa(value: Price, precision: u8, round: Round) -> Result<Price> {
        if value.is_zero() {
            return Ok(dec!(0));
        }
    ...
    fn get_precision_digits_by_fractional(value: Price, precision: u8) -> Result<i8> {
        if precision <= 0 {
            bail!(
        ...
        let floor_digits = precision as i8 - integral_digits;
